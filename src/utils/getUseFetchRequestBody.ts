...
import { isDictionaryLike } from './isDictionaryLike'
...
export const getUseFetchRequestBody = <Body extends RequestBody>(
  ...
  const shouldMergeObjects = isDictionaryLike(rawBaseBody) && isDictionaryLike(rawTriggerBody)

  const rawBody = shouldMergeObjects
    ? {
        ...rawBaseBody,
        ...rawTriggerBody,
      }
